// PROTOTYPE — throwaway. Variant B2 "Week grouping": the timeline rail from
// variant B, but entries group by week instead of day so long histories read
// in chapters; entry times gain a weekday prefix to carry the day.
...
/** Monday-start weeks, matching ISO conventions. */
const WEEK_OPTIONS = { weekStartsOn: 1 } as const;

function getWeekLabel(createdAt: number) {
  const date = new Date(createdAt);
  const now = new Date();

  if (isSameWeek(date, now, WEEK_OPTIONS)) return "This week";
  if (isSameWeek(date, subWeeks(now, 1), WEEK_OPTIONS)) return "Last week";
  ...
  return isCurrentYear ? range : `${range}, ${format(end, "yyyy")}`;
}

function groupLogsByWeek(logs: ActivityLogEntry[]) {
  const groups = new Map<string, ActivityLogEntry[]>();

  for (const log of [...logs].sort((a, b) => b.createdAt - a.createdAt)) {
    const key = format(
      startOfWeek(new Date(log.createdAt), WEEK_OPTIONS),
      "yyyy-MM-dd",
    );
  ...
  return [...groups.entries()].map(([key, groupLogs]) => ({
    key,
    label: getWeekLabel(groupLogs[0]!.createdAt),
    logs: groupLogs,
  }));
}

/** Week groups are coarse, so non-today times carry the weekday: "Mon 3:12 PM". */
function formatEntryTime(createdAt: number) {
  const date = new Date(createdAt);
  return format(date, isToday(date) ? "h:mm a" : "EEE h:mm a");
}

export function ActivityLogRailWeekGroups({
  logs,
  onAddNote,
          ...
          <form onSubmit={handleSubmit}>
            <FieldGroup className="gap-0">
              <Field data-disabled={isPending || undefined}>
                <FieldLabel
                  htmlFor="activity-log-rail-week-note"
                  className="sr-only"
                >
                  Activity log note
                </FieldLabel>
                <InputGroup>
                  <InputGroupTextarea
                    id="activity-log-rail-week-note"
                    value={noteText}
...
function RailBody({ logs }: { logs: ActivityLogEntry[] | undefined }) {
  ...
  return (
    <div className="flex flex-col">
      {groupLogsByWeek(logs).map((group) => (
        <section
          key={group.key}
          aria-label={group.label}
          className="flex flex-col"
        >
          <WeekMarker label={group.label} />
          {group.logs.map((log) =>
...
/** Week label to the right of the rail, directly above its log items; the
 *  rail runs through uninterrupted. */
function WeekMarker({ label }: { label: string }) {
  return (
