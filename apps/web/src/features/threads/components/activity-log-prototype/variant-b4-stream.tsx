// PROTOTYPE — throwaway. Variant B4 "Ungrouped stream": the timeline rail with
// no day headers — one unbroken flow where each entry's timestamp carries the
// date, and day boundaries only get a touch of extra breathing room.
...
/** Date-carrying timestamp: time only for today, "Yesterday · time" for
 *  yesterday, otherwise "Jul 24 · time" (with year when not this year). */
function formatStreamTimestamp(createdAt: number) {
...
function isDayBoundary(previous: ActivityLogEntry, current: ActivityLogEntry) {
  return (
    format(new Date(previous.createdAt), "yyyy-MM-dd") !==
    format(new Date(current.createdAt), "yyyy-MM-dd")
  );
}

export function ActivityLogRailStream({
  logs,
  onAddNote,
          ...
          <form onSubmit={handleSubmit}>
            <FieldGroup className="gap-0">
              <Field data-disabled={isPending || undefined}>
                <FieldLabel
                  htmlFor="activity-log-rail-stream-note"
                  className="sr-only"
                >
                  Activity log note
                </FieldLabel>
                <InputGroup>
                  <InputGroupTextarea
                    id="activity-log-rail-stream-note"
                    value={noteText}
  ...
  return (
    <div className="flex flex-col">
      {sorted.map((log, index) => {
        ...
        const crossesDay =
          previous !== undefined && isDayBoundary(previous, log);

        return isAutomaticActivityLogEntry(log) ? (
          <AutomaticTick key={log._id} log={log} crossesDay={crossesDay} />
        ) : (
          <NoteCard key={log._id} log={log} crossesDay={crossesDay} />
        );
      })}
    </div>
  );
...
function NoteCard({
  ...
  return (
    <div className={cn("relative py-1.5", ENTRY_PAD, crossesDay && "mt-3")}>
      <span
        aria-hidden
        className={cn(
...
function AutomaticTick({
  ...
  return (
    <div className={cn("relative py-1", ENTRY_PAD, crossesDay && "mt-3")}>
      <span
        aria-hidden
        className={cn(
...
function EntryTime({ createdAt }: { createdAt: number }) {
  return (
    ...
    >
      {formatStreamTimestamp(createdAt)}
    </time>
  );
}
