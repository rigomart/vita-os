// PROTOTYPE — throwaway. Copy of ../../plan/plan-lane.tsx for Variant C:
// the lane header links to the Area page and non-healthy lanes always carry
// their condition label, since the canvas is the whole dashboard here.
...
/** Same wording as the area condition overview this variant replaces. */
...
function AreaLaneHeader({ incoming, lane }: { incoming: boolean; lane: Lane }) {
  ...
  return (
    ...
    >
      ...
      />
      {/* The whole header cell is the Area link. Drag never starts here —
          only chips are draggable — so navigation and drag stay disjoint. */}
      <Link
        to="/$areaSlug"
...
function LaneStatus({ incoming, lane }: { incoming: boolean; lane: Lane }) {
  ...
  // With the card grid gone, the lane header is the only place condition
  // shows — so non-healthy lanes always carry the label, even when empty.
