// PROTOTYPE — throwaway. Copy of ../../plan/plan-filters.tsx for Variant B:
// non-healthy Area chips carry a small condition glyph, so attention is
// scannable from the filter row now that the area-health section is gone.
    ...
    <div className="flex flex-wrap items-center gap-1.5">
      ...
      {areas.map((area) => {
        ...
        const ConditionGlyph =
          area.condition === "healthy" ? null : conditionIcons[area.condition];
        return (
          <button
            key={area.id}
            type="button"
