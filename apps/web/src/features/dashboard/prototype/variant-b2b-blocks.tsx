// PROTOTYPE — throwaway (issue #268). Variant B2B: canon calendar schedule with chips restyled as solid calendar event blocks.
...
/**
 * Plan — calendar schedule, event blocks (phone).
 *
 * Structurally identical to the canon B2 schedule: 48px date rail, sticky month
 * band, hairline rows for empty near days, folded gap runs, floating Today pill,
 * local-state drops. The one thing that changes is the *chip*.
 *
 * B2's chip is a card that sits on the day. B2B's chip is an **event block**: a
 * filled, rounded slab tinted with the Area's condition fill and cut down its
 * left edge by a 4px full-strength accent — the shape Google Calendar and Apple
 * Calendar have trained every thumb on. Two dense lines (icon + title, then the
 * next move) instead of a three-part card, so a day full of work reads as a day
 * full of blocks rather than a stack of notes. Inbox Tasks, which have no Area
 * and so no condition, stay a neutral dashed block — an unscheduled placeholder,
 * the way a calendar draws a tentative event.
 *
 * This is a prototype: drops write to a local `moves` map, never to Convex.
 */
export function PlanVariantBlocks({
  areas,
  currentDate,
  tasks,
  threads,
  ...
  function handleDragStart(event: DragStartEvent) {
    const data = event.active.data.current as BlockDrag | undefined;
    if (!data) return;
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    const source = event.active.data.current as BlockDrag | undefined;
    const over = event.over?.data.current as SlotDrop | undefined;
...
/**
 * One calendar day: rail on the left, event blocks stacked to its right.
 *
 * An empty day keeps the row and loses only the blocks — the content column
        ...
        className={cn(
          ...
          armed
            ...
            : isToday
              ...
              : "border-border/45",
          // Blocks butt up close, the way stacked events do in a day column.
          empty ? "h-6" : "flex flex-col gap-[3px] py-1.5",
        )}
      >
        {items.map((item) => (
          <CalendarBlock
...
/**
 * The tinted slab. `-fill` tokens at low alpha, not the text-safe base tokens:
 * the fills are theme-invariant hues chosen to survive both the warm cream and
 * the dark ink surface, so one alpha reads as the same event colour in both.
 */
const blockFill: Record<Condition, string> = {
  critical: "bg-condition-critical-fill/20",
  needs_attention: "bg-condition-attention-fill/22",
  healthy: "bg-condition-healthy-fill/16",
};

/** The accent edge, at full strength — the part you actually read the colour off. */
const blockEdge: Record<Condition, string> = {
 ...
 *
 * Anatomy, outside in: an opaque rounded shell (it matters only when lifted —
 * a translucent tint dragged over the page would smear), a 4px full-strength
 * condition edge down the left, and a body tinted with that condition's `-fill`
 * token. Inside, two dense lines: Area icon plus title, then the next move (or
 * the waiting age, in the overdue section) in muted type. Inbox Tasks have no
 * Area and therefore no condition, so they take a dashed neutral block.
 */
function EventBlock({
  areaById,
  item,
  ...
  // Overdue outranks the Area's own condition: in that section the block is
  // about the wait, not about the Area.
  const condition: Condition = waiting
    ? "needs_attention"
    : (area?.condition ?? "healthy");

  const sub = waiting ? waitingLabel(item.date!, now) : item.nextMove;

  return (
    <div
      className={cn(
        "relative w-full overflow-hidden rounded-lg text-left",
        lifted && "bg-surface-1 shadow-lg ring-1 ring-foreground/10",
      )}
    >
      <div
        className={cn(
          "flex w-full items-start gap-1.5 py-1.5 pr-2 pl-3",
          isTask
            ? "border border-dashed border-border bg-surface-2/70"
            : blockFill[condition],
        )}
      >
        <span
          ...
          className={cn(
            ...
            isTask ? "bg-border" : blockEdge[condition],
          )}
        />

        <span className="mt-[3px] shrink-0 text-foreground/70">
          {isTask ? (
        ...
        <span className="min-w-0 flex-1">
          ...
          {sub && (
            <span className="mt-px flex min-w-0 items-center gap-1 text-[10px] leading-[1.3] text-muted-foreground">
              {!waiting && (
...
function CalendarBlock({
  ...
  const { attributes, isDragging, listeners, setNodeRef } = useDraggable({
    id: item.id,
    data: { itemId: item.id, slotKey } satisfies BlockDrag,
  });
  ...
  return (
    <button
      ...
      className={cn(
        // `touch-manipulation`, not `touch-none`: a swipe that starts on a block
