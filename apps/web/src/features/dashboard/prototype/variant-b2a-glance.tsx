// PROTOTYPE — throwaway (issue #268). Variant B2A: canon calendar schedule + glanceability and drag fixes from review.
...
/**
 * Plan — calendar schedule, shipped (phone).
 *
 * Same design as variant B2: a Google-Calendar Schedule agenda with a 48px date
 * rail, sticky month bands, hairline rows for the empty near days, folded "quiet
 * days" past the horizon and a floating Today pill. Nothing about the visual
 * language moves. What moves is everything three reviews caught:
 *
 * - the **tally** rides inside the sticky band, so "what am I holding" survives
 *   scrolling instead of living above the fold;
 * - the **filter chips** sort worst-condition-first and wear their condition, so
 *   the row is a triage strip rather than an alphabet;
 * - a day stacks its chips **worst-first**, so the top of a busy day is the part
 *   that needs you;
 * - a drop **follows** the item: the landing row is opened and scrolled into
 *   view rather than vanishing somewhere below the fold;
 * - the header inset is **measured**, not guessed;
 * - an empty day's hairline keeps its 24px look but drops with ~40px of hit;
 * - hovering a folded gap for 400ms **opens it mid-drag**, so a long throw stops
 *   being abort → tap → re-lift.
 *
 * Still a prototype: drops write to a local `moves` map, never to Convex.
 */
export function PlanVariantGlance({
  areas,
  currentDate,
  tasks,
  threads,
  ...
  /**
   * The slot a drop just landed on. Set in `onDragEnd` and consumed by an effect
   * on the *next* commit — by then the row exists (a gap may have opened, a day
   * may have grown out of a fold) and can be measured honestly. Reading refs
   * from a dep-less effect would measure the layout the drop replaced.
   */
  const [pendingScroll, setPendingScroll] = useState<string | null>(null);

  const todayRef = useRef<HTMLDivElement | null>(null);
  /** slot key → its row node, so a drop can scroll to where the chip went. */
  ...
  /** The desktop header's idiom, on the one line a phone can spare. */
  const tally = useMemo(
    () => ({
      today: schedule.byDay.get(dayKey(0))?.length ?? 0,
      undated: schedule.none.length,
      waiting: schedule.overdue.length,
    }),
    [schedule],
  );
  ...
  const [bandHeight, setBandHeight] = useState(BAND_FALLBACK);

  /**
   * The sticky block is the app bar plus the month band pinned under it. B2 hard
   * coded 80px and drifted the moment the band gained a line; measure instead.
   */
  const attachBand = useCallback((node: HTMLDivElement | null) => {
    bandRef.current = node;
    if (node) setBandHeight(node.offsetHeight);
  }, []);

  const headerInset = useCallback(
    () => APP_BAR + (bandRef.current?.offsetHeight ?? BAND_FALLBACK),
    [],
  );
  ...
  const scrollToToday = useCallback(
    (smooth: boolean) => {
      ...
      const top =
        node.getBoundingClientRect().top + window.scrollY - headerInset();
  ...
  useEffect(() => {
    const node = todayRef.current;
    if (!node) return;
    // rootMargin has to be a static string, so this one re-arms on the measured
    // band height rather than reading it per-callback.
    const observer = new IntersectionObserver(
      ([entry]) => setTodayInView(entry.isIntersecting),
      { rootMargin: `-${APP_BAR + bandHeight}px 0px -25% 0px` },
    );
  ...
  const scrollToSlot = useCallback(
    (key: string) => {
      ...
      // Already comfortably on screen: moving the page would be the surprise.
      if (box.top >= inset && box.bottom <= window.innerHeight - BOTTOM_INSET) {
        return;
      }
      window.scrollTo({
        behavior: "smooth",
        top: box.top + window.scrollY - inset - 8,
      });
    },
    [headerInset],
  );
  ...
  useEffect(() => {
    ...
    // One frame after the commit: layout has settled, the row has a rect.
    const frame = requestAnimationFrame(() => {
      scrollToSlot(pendingScroll);
      setPendingScroll(null);
    });
    return () => cancelAnimationFrame(frame);
  }, [pendingScroll, scrollToSlot]);
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    const over = event.over?.data.current as SlotDrop | undefined;
    // A gap row is a hover target, not a drop target: it has no slot key.
    const target = over?.slotKey;
    if (!source || target == null) return;
    // The past is not a target — the same rule the canvas enforces.
    if (target === "overdue") return;
    if (target === source.slotKey) return;

    if (target === "none") setShowNoDate(true);
    else expandGapAround(Number(target.slice(1)));

    setMoves((previous) => {
      const next = new Map(previous);
      next.set(source.itemId, target);
      return next;
    });
  ...
  /** Landing inside a fold: unfold it so the chip has somewhere visible to be. */
  function expandGapAround(offset: number) {
    for (const row of schedule.rows) {
      if (row.kind !== "gap") continue;
      if (offset >= row.from && offset <= row.to) expandGap(row.from);
    }
  }
  ...
  const firstMonthKey = schedule.rows.find((row) => row.kind === "month")?.key;

  /* ------------------------------------------------------------ render -- */

  return (
    <section aria-label="Plan" className="flex flex-col gap-2.5">
      {/* The narrowing that lane headers do on desktop — worst Area first. */}
      ...
      <DndContext
        ...
        sensors={sensors}
        // Rows revealed mid-drag (a gap that unfolds under the finger) only get
        // droppable rects if dnd-kit keeps measuring after drag start.
          ...
          {schedule.rows.map((row) => {
            if (row.kind === "month") {
              return (
                <MonthBand
                  key={row.key}
                  bandRef={row.key === firstMonthKey ? attachBand : undefined}
                  label={row.label}
                  tally={tally}
                />
              );
...
/** A day/bay carries a `slotKey`; a gap row is hover-only and carries neither. */
interface SlotDrop {
  gapFrom?: number;
  slotKey?: string;
}
...
/** The sticky app bar the month band pins under. */
const APP_BAR = 48;
/** Used only before the band has been measured (first frame, empty list). */
const BAND_FALLBACK = 44;
/**
 * Coarse: the mobile tab bar plus the DEV toolbar own the bottom edge. Only
 * decides "is the landing row comfortably visible", so a constant is enough.
 */
const BOTTOM_INSET = 96;

/** How long a drag must hover a fold before it opens under the finger. */
const GAP_HOVER_MS = 400;
...
/** No Area (an inbox Task) sorts after every Area. */
const NO_AREA_RANK = 3;
 ...
 *
 * Within a day, chips stack **worst condition first**: on a phone the top of a
 * day is all you see at a glance, so it has to be the part that needs you.
 */
function buildSchedule(
  ...
  function rank(item: PlanItem): number {
    const area = item.areaId == null ? undefined : areaById.get(item.areaId);
    return area ? CONDITION_RANK[area.condition] : NO_AREA_RANK;
  }

  function byConditionThenDateThenTitle(a: PlanItem, b: PlanItem): number {
    return rank(a) - rank(b) || byDateThenTitle(a, b);
  }

  for (const bucket of byDay.values()) {
    bucket.sort(byConditionThenDateThenTitle);
  }
  overdue.sort(byConditionThenDateThenTitle);
  none.sort(byConditionThenDateThenTitle);
...
/** Off-state is shared by every condition: a filtered-out Area is just quiet. */
const CHIP_OFF =
  "bg-transparent text-muted-foreground/45 ring-border/40 hover:text-muted-foreground";

/**
 * Condition tint for a filter chip. Critical and attention take the vivid fill
 * tokens — on cream, low-alpha tints vanish — while healthy stays deliberately
 * signal-free: "nothing wrong here" is the message.
 */
const chipTone: Record<Condition, string> = {
  critical:
    "bg-condition-critical-fill text-condition-critical-fill-foreground ring-transparent",
  healthy: "bg-surface-2 text-foreground ring-border hover:bg-surface-3",
  needs_attention:
    "bg-condition-attention-fill text-condition-attention-fill-foreground ring-transparent",
};

/**
 * Area filter chips, triage-ordered.
 *
 * Same contract as the shared `AreaFilterChips` — `null` means everything, the
 * first tap isolates, emptying falls back to all — but the row is sorted worst
 * condition first and each pill wears its Area's condition. On a phone the chip
 * row is the only place the Area layer survives, so it may as well carry the
 * signal the desktop lane headers do.
 */
function ConditionFilterChips({
  active,
  areas,
  counts,
  onChange,
  ...
  const needsMe = ordered.filter((area) => area.condition !== "healthy");
  const needsMeActive =
    active != null &&
    needsMe.length > 0 &&
  ...
  return (
    <div className="flex items-center gap-1.5">
      <button
        ...
        className={cn(
          "h-7 shrink-0 rounded-full px-2.5 text-[11px] font-medium transition-colors",
          active == null
            ? "bg-foreground text-surface-1"
      ...
      {needsMe.length > 0 && (
        <button
          type="button"
          aria-pressed={needsMeActive}
          onClick={() =>
            onChange(
              needsMeActive ? null : new Set(needsMe.map((area) => area.id)),
            )
          }
          className={cn(
            "h-7 shrink-0 rounded-full px-2.5 text-[11px] font-medium transition-colors",
      ...
      {ordered.map((area) => {
        ...
        return (
          <button
            ...
            className={cn(
              "flex h-7 shrink-0 items-center gap-1.5 rounded-full px-2.5 text-[11px] font-medium ring-1 transition-colors",
              on ? chipTone[area.condition] : CHIP_OFF,
            )}
          >
            <AreaIcon
              icon={area.icon}
            ...
            <span
              className={cn(
                "tabular-nums",
                on ? "opacity-65" : "text-muted-foreground/40",
              )}
            >
...
/**
 * The one piece of chrome that spans the full width, and the only one that is
 * always on screen — so the tally rides here rather than above the fold. Month
 * label and tally together stay under ~44px, one thumb-scroll of chrome.
 */
function MonthBand({
  bandRef,
  label,
  ...
  return (
    <div
      ref={bandRef}
      className="sticky top-12 z-20 -mx-1 bg-surface-1/95 px-1 pt-2 pb-1 backdrop-blur-sm"
    >
      <div className="flex items-center gap-2">
        <span className="font-heading text-[11px] leading-none font-semibold tracking-[0.09em] text-muted-foreground uppercase">
          {label}
        </span>
        <span aria-hidden className="h-px flex-1 bg-border/60" />
      </div>
      <p className="flex items-center gap-x-1.5 text-xs leading-5 text-muted-foreground">
        <span
          className={cn(
            "tabular-nums",
            tally.waiting > 0 && "font-medium text-condition-attention",
          )}
        >
 ...
 * becomes a hairline, so the day stays a visible, droppable place. The *hit*
 * area, though, is not the hairline: the droppable node pads itself 8px each
 * way and pulls the padding back out with a negative margin, so a 24px-looking
 * row catches ~40px of thumb without moving a pixel of layout. Neighbouring
 * empties overlap a little; `pointerWithin` breaks the tie by rect centre,
 * which lands on the nearer day.
 */
function DayRow({
  chrome,
  items = EMPTY,
  ...
  return (
    <div
      ...
      className={cn("relative", empty && "z-[1] -my-2 py-2")}
    >
      <div
        className={cn(
 ...
 *
 * It is also a droppable — not one that accepts anything, one that *reacts*:
 * hold a dragged chip over it for 400ms and the fold opens in place, turning
 * the thin days underneath into live targets without ever putting the chip
 * down. Before this, a long throw meant abort, tap the fold, lift again.
 */
function GapRow({
  chrome,
  from,
  ...
  const { isOver, setNodeRef } = useDroppable({
    id: `gap::${from}`,
    data: { gapFrom: from } satisfies SlotDrop,
  });
  const dragging = chrome.drag != null;
  const hovering = dragging && isOver;

  useEffect(() => {
    if (!hovering || open) return;
    const timer = setTimeout(() => onExpand(from), GAP_HOVER_MS);
    return () => clearTimeout(timer);
  }, [from, hovering, onExpand, open]);
  ...
  return (
    <div className="flex flex-col">
      <button
        ...
        onClick={onToggle}
        className={cn(
          "flex items-center border-t border-dashed border-border/50 py-1.5 text-left transition-colors",
          hovering && !open && "rounded-md bg-brand-gold-strong/10",
        )}
      >
        <span
          aria-hidden
        ...
        <span className="flex flex-1 items-center gap-1 pl-1 text-[10px] text-muted-foreground/55">
          ...
          {hovering && !open && (
            <span className="ml-1 text-muted-foreground/70">hold to open</span>
          )}
        </span>
      </button>
        ...
        offsets.map((offset) => {
          ...
          return (
            <div key={dayKey(offset)} className="flex flex-col">
              {crossesMonth && (
                <MonthBand label={monthLabel(at)} tally={chrome.tally} />
              )}
