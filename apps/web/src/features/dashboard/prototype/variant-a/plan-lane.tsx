// PROTOTYPE — throwaway. Variant A copy of plan-lane.tsx: the Area lane header
// links to the area page, and non-healthy lanes always announce their
// condition in the status line.
...
/**
 * PROTOTYPE change: the whole header content is a Link to the area page. The
 * header is never a drag source (only chips are draggables) and never a drop
 * target, so navigation cannot collide with dnd-kit — a drag that starts on a
 * chip and merely passes over the header keeps working, and a plain click on
 * the header navigates. The affordance stays subtle: the area name warms to
 * the primary color on hover, and keyboard focus draws the app's ring inset.
 */
  ...
  return (
    ...
    >
      ...
      <Link
        ...
        className="group/lane relative flex items-start gap-2.5 rounded-md py-2 pr-3 pl-3.5 outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-inset"
      >
        <span
          className={cn(
...
function LaneStatus({ incoming, lane }: { incoming: boolean; lane: Lane }) {
  ...
  // PROTOTYPE change: a non-healthy Area always leads its status line with the
  // condition icon and label (Critical / Needs you), even when the lane holds
  // no open or planned items — the condition never hides behind emptiness.
