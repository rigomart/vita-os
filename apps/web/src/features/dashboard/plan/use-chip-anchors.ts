...
/** Every chip in a container, plus the container's own box to draw inside. */
 ...
 * container's own box — the numbers an overlay drawn at `absolute inset-0`
 * needs to meet the chips exactly, which only the live layout can supply.
 *
 * Re-measures on mount, on container resize, and on any DOM change beneath it,
 * all collapsed into one animation frame so a drag or a filter change costs a
 * single pass. The container must be `relative`.
 *
 * Where there is no layout to read — server rendering, jsdom — this degrades to
 * zero anchors rather than guessing, so callers render nothing.
 */
