...
const railTone: Partial<Record<ColumnKey, string>> = {
  ...
  tomorrow: "bg-brand-accent-foreground/50",
};
...
/**
 * The chip while its column is zoomed into day slots.
 *
 * A day slot is ~40px wide, so the title cannot survive — what survives is the
 * item's *position*: which exact day it sits on. Zoom trades labels for
 * precision, and hands the label back on hover and in the drag caption.
 */
export function MiniChipSurface({
  item,
  selected,
}: {
  item: PlanItem;
  ...
  return (
    <div
      title={item.title}
      className={cn(
        "relative flex h-[26px] w-full items-center overflow-hidden rounded-md border pr-0.5 pl-1.5 transition-colors",
        isTask
          ? "border-dashed border-border bg-surface-2/60"
          : "border-border/70 bg-surface-2 hover:border-border hover:bg-surface-3/70",
        selected && "border-ring/60 bg-surface-3 ring-2 ring-ring/40",
      )}
    >
      {!isTask && (
        <span
          aria-hidden
          className="absolute inset-y-1 left-0 w-[2px] rounded-r-full bg-border"
        />
      )}
      <span
        className={cn(
...
export function PlanChip({
  ...
  const hint =
    item.kind === "thread" && item.nextMove
      ? `${item.title} — Next Move: ${item.nextMove}`
      : item.title;
  ...
  return (
    ...
    >
      {mini ? (
        <MiniChipSurface item={item} selected={selected} />
      ) : (
        <ChipSurface
          column={column}
          density={density}
