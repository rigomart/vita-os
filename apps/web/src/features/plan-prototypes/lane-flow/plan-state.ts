...
/**
 * Local, undoable state for the Lane flow canvas.
 *
...
export type PlanAction =
  ...
  | {
      /** Undefined when the drop only crossed lanes. */
...
export function planReducer(state: PlanState, action: PlanAction): PlanState {
  switch (action.type) {
    ...
    case "drop": {
      ...
      if (action.toAreaId == null && !action.reschedule) return state;

      const change: Partial<PlanItem> = {};
      if (action.toAreaId != null) change.areaId = action.toAreaId;
