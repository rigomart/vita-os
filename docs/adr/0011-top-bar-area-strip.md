...
Status: Amended by ADR 0018 — the strip moves into the floating identity cluster and carries Condition as the hexagon's own colour plus an open-Thread count. The one-click jump and the 1..9 digits are unchanged. Superseded by ADR 0021 — the strip is removed; `1..9` now select the matching Dashboard Area filter and `0` returns to All.
