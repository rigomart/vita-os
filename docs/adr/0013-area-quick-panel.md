...
Status: Amended by ADR 0014 — the Dashboard Condition strip replaces the retired Plan lane header as the panel trigger; the top-bar Area strip remains pure navigation. An absent Standard now contributes no section, and the scoped capture button is labeled "New Thread." Amended by ADR 0018 — the Dashboard Condition strip trigger is retired with the Dashboard header. The panel is unchanged and still opens from the palette's Area rows.
