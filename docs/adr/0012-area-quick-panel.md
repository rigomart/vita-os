...
The **Dashboard** could show **Area** state but never touch it: the status bar names the Area and the reason, **Plan** lanes carry the **Condition**, yet recording the judgment that awareness just produced — downgrading an Area after handling the reason, escalating another, checking what "healthy" even means there, or capturing a **Thread** prompted by the state — required leaving for the Area page and finding the way back. Left-clicking a Plan lane header now opens an **Area Quick Panel** over the Dashboard instead of navigating: the Condition select pill, the Area's **Standard** as read-only text (a quiet empty state pointing at the Area page when unwritten), a "New Thread in {Area}" action that pre-scopes the existing create flow and opens the created Thread in place in the Thread rail (ADR 0007), and an "Open {Area} →" link. The command palette gains a drill-in per Area row offering the same action set, with Enter still jumping straight to the Area page. One shared **Area Actions model** defines that set and its handlers, and both summon surfaces consume it, so the two cannot drift apart. This is the Areas instance of the app's anti-navigation arc — ADR 0006 (palette-first), ADR 0007 (in-place Thread rail), ADR 0010 (top-bar Area strip), ADR 0011 (in-place Inbox).
...
- **Make every Area representation actionable app-wide**: strip hexagons, status-bar entries, and lane headers all growing an action affordance. Rejected: it turns the app's two pure-navigation surfaces into decision points, taxing the glance-and-go path that ADR 0010 exists to make instant, and spreads the action set across surfaces with no single owner.
...
- **The panel is summoned; it adds zero ambient state.** Nothing new stands on screen. The standing "no second awareness surface" rule (ADRs 0006, 0010, 0011) holds — the Dashboard remains the only place Area state is ambiently readable.
...
- **Removing the header's link role is safe because the route is redundant.** Since ADR 0010 the Dashboard has carried two same-screen one-click routes to any Area page — the strip (plus its `1..9` keys) and the palette. The strip and the status bar keep that pure-navigation role and gain no actions.
...
- Condition is presented with the established condition tokens and never by color alone (ADR 0008); the panel reuses the existing select pill rather than restating the ramp.
