...
Status: Amended by ADR 0017 — the Dashboard keeps this date-state language but lays it out as time columns with an unscheduled margin, and acts on Follow-ups in place; the open ordering question is settled there as "a date outranks an undated Next Move". Notes and the Area inventory continue sharing the interactive row implementation.
