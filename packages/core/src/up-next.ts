import type { ThreadState } from "@vita-os/contracts";
 ...
 * Move. Plain text — no dates, no done states — with queue semantics: the Next
 * Move is the front of the line, and emptying that slot promotes the next move
 * into it.
 *
 * Absent and empty mean the same thing, so only absence is ever stored: a
 * Thread with nothing lined up reads exactly as it did before Up Next existed.
 */

/**
 * Up Next writes are open-Thread edits. A resolved Thread discarded its line at
 * resolution; writing to it would refill the Next Move slot through the
 * promotion invariant and resurrect attention state that reopening must not
 * restore.
 */
export function requireOpenForUpNext(thread: { state: ThreadState }): void {
  if (thread.state !== "open") {
    throw new ConflictError("Cannot line up moves on a resolved thread");
  }
}
