...
bun run lint         # Check linting + formatting (no auto-fix)
bun run check        # Lint + format + organize imports with auto-fix
...
- `src/main.tsx` — Entry point. `ConvexProvider` wraps `RouterProvider`.
